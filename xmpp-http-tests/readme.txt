Setup:
	...
	4. HTTP Requests
		...
		d. Using curl, input the command
			...
			curl -v -d @request1.xml -H "Content-type: application/xml" http://localhost:9000/requests
		...
		f. Using curl, input the command
			...
			curl -v -d @created1.xml -H "Content-type: application/xml" http://localhost:9000/requests
